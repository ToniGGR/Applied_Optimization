...
def compare_Efficiency (testruns=10):
    ...
    for run in range(testruns):
        ...
        failure_ls[1] = "g" if nedermead_perform(initial_guess=x_test )["PL"] < 1000 and failure_ls[1] else "r"
        list_eff[2][run] = cobyla_perform(initial_guess=x_test)["E"] * -1
        failure_ls[2] = "g" if cobyla_perform(initial_guess=x_test)["PL"] < 1000 and failure_ls[2] else "r"
        list_eff[3][run] = cobyqa_perform(initial_guess=x_test)["E"] * -1
        failure_ls[3] = "g" if cobyqa_perform(initial_guess=x_test)["PL"] < 1000 and failure_ls[3] else "r"
        list_eff[4][run] = slsqp_perform(initial_guess=x_test)["E"] * -1
        failure_ls[4] = "g" if slsqp_perform(initial_guess=x_test)["PL"] < 1000 and failure_ls[4] else "r"
        list_eff[5][run] = direct_perform(initial_guess=x_test)["E"] * -1
        failure_ls[5] = "g" if direct_perform(initial_guess=x_test)["PL"] < 1000 and failure_ls[5] else "r"
